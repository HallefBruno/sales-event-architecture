...
1. **[Order Service](./order-service) (Producer):** Recebe a requisição de um novo pedido, processa a regra de negócio e publica um evento JSON na Exchange principal.
2. **[Notification Service](./notification-service) (Consumer):** Consome os eventos da fila de forma assíncrona e processa o envio de e-mails fictícios de confirmação.
...
```mermaid
graph LR
    A[Cliente] -->|POST /pedidos| B(Order Service)
    B -->|Publica com RK: order-created| C[Exchange: sales.v1.events]
    C -->|Direciona| D[Fila: sales.v1.send-email]
    D -->|Consome| E(Notification Service)
    E -->|Sucesso| F[E-mail Enviado]
```

## 🛡️ Resiliência, Tolerância a Falhas e DLX

O **Notification Service** utiliza estratégias avançadas de resiliência industrial para garantir a integridade dos dados e proteção do Broker:
...
*   `/order-service`: API REST responsável pela criação de pedidos e publicação de eventos.
*   `/notification-service`: Worker responsável pelo consumo da fila e disparo de notificações.
...
4. Abra e execute o projeto **Notification Service** (porta padrão: `8081`).
